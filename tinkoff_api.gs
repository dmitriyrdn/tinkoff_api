...
function _GetTickerNameByFIGI(figi='YNDX') {
...
function TI_GetLastPrice(ticker='YNDX') {
...
function TI_GetPortfolio(account_number) {
  const portfolio = tinkoffClientV2._GetPortfolio(TI_GetAccounts(account_number));
  const values = []
